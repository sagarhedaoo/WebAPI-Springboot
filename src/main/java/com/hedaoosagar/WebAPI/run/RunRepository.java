...
import com.hedaoosagar.WebAPI.WebApiApplication;
...
public class RunRepository {
    ...
    public void create(Run run) {
        var updated = jdbcClient.sql("INSERT INTO Run(id, title, started_on, ended_on, miles, location) values (?,?,?,?,?,?)")
                .param(List.of(run.id(), run.title(), run.startedOn(), run.endedOn(), run.miles(), run.location().toString()))
                .update();
    ...
    public void update(Run run, Integer id) {
        var updated = jdbcClient.sql("UPDATE run SET title = ?, started_on = ?, ended_on = ?, location = ? WHERE id = ?")
                .params(List.of(run.title(), run.startedOn(), run.endedOn(), run.location().toString(), id))
                .update();
